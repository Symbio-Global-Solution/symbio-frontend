...
import { FaUserPlus, FaEdit, FaTrash, FaSync, FaTimes, FaEnvelope, FaCalendarAlt, FaMoneyBillWave, FaIdBadge, FaUserTie, FaListUl } from 'react-icons/fa';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 p-6 rounded-2xl h-fit shadow-lg sticky top-24">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="text-xs font-bold opacity-70 uppercase mb-1 block">Nome Completo</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="text-xs font-bold opacity-70 uppercase mb-1 block">Email Corporativo</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold opacity-70 uppercase mb-1 block">Admissão</label>
                  <input 
                    type="date" 
                    value={dt_admissao}
                ...
                <div>
                  <label className="text-xs font-bold opacity-70 uppercase mb-1 block">Salário (R$)</label>
                  <input 
                    type="number" 
                    step="0.01"
                    value={salario}
              ...
              <div>
                <label className="text-xs font-bold opacity-70 uppercase mb-1 block">ID do Cargo</label>
                <div className="relative mb-2">
                    <span className="absolute left-3 top-2.5 text-gray-400"><FaIdBadge /></span>
                    <input 
                    type="number" 
                    value={id_cargo}
                        ...
                        <ul className="space-y-1">
                            {cargosDisponiveis.map(cargo => (
                                ...
                                >
                                    ...
                                    <span className="font-mono font-bold bg-gray-200 dark:bg-white/10 px-1 rounded">{cargo.id}</span>
                                </li>
                            ))}
                        </ul>
                    ) : (
                        <p className="text-xs text-gray-400 italic">Nenhum cargo carregado.</p>
                    )}
                </div>
              ...
              <div className="flex gap-2 pt-2">
                <button 
                  type="submit" 
                  className={`flex-1 font-bold py-3 rounded-lg transition-colors text-white ${idEdicao ? 'bg-yellow-500 hover:bg-yellow-600' : 'bg-purple-600 hover:bg-purple-700'}`}
                >
