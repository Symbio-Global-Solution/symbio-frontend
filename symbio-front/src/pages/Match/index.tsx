  ...
  const handleAnalyze = async () => {
    ...
    try {
      const data = await apiService.get<any>(
        `/match/${selectedColab}/${selectedVaga}`
      );
      ...
      <div className="max-w-3xl w-full mt-6">
        <div className="text-center mb-10 animate-fade-in">
          ...
          <p className="text-gray-500 dark:text-gray-400">
            Selecione os parâmetros para a IA calcular a aderência.
          </p>
        ...
        {initialLoading ? (
          <div className="flex flex-col items-center justify-center py-20 animate-pulse">
            ...
            <p className="text-gray-500 dark:text-gray-400 font-medium">
              Conectando ao ecossistema Symbio...
            </p>
          </div>
        ) : (
          <div className="bg-white dark:bg-white/5 backdrop-blur-md border border-gray-200 dark:border-white/10 p-8 rounded-3xl shadow-xl animate-fade-in-up">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div className="group">
                <label className="text-xs font-bold ml-1 mb-2 block uppercase text-gray-400 tracking-wider">
                  Colaborador
                </label>
            ...
            {result && (
              <div className="mt-8 overflow-hidden rounded-2xl animate-fade-in-up">
                ...
                >
                  ...
                  <p className="text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400 mb-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
                    <div className="bg-white/60 dark:bg-black/20 p-4 rounded-xl">
                      <p className="text-xs font-bold text-gray-500 uppercase mb-3 flex items-center gap-2">
                        <FaCheckCircle className="text-green-500" /> Pontos
                        Fortes
                      </p>
                      <div className="flex flex-wrap gap-2">
                        {result.skillsMatch?.length > 0 ? (
                          result.skillsMatch.map((s, i) => (
                            <span
                              key={i}
                              className="text-xs bg-green-100 dark:bg-green-500/10 text-green-700 dark:text-green-300 px-2 py-1 rounded border border-green-200 dark:border-green-500/20"
                            >
                              {s}
                            </span>
                          ))
                      ...
                      <div className="flex flex-wrap gap-2">
                        {result.gaps?.length > 0 ? (
                          result.gaps.map((g, i) => (
                            <span
                              key={i}
                              className="text-xs bg-orange-100 dark:bg-orange-500/10 text-orange-700 dark:text-orange-300 px-2 py-1 rounded border border-orange-200 dark:border-orange-500/20"
                            >
                              {g}
                            </span>
                          ))
                  ...
                  <div className="mt-6 pt-4 border-t border-gray-200 dark:border-white/5">
                    <p className="text-sm italic text-gray-600 dark:text-gray-400">
                      <span className="font-bold text-purple-600 dark:text-purple-400 not-italic mr-2">
                        Insight IA:
                      </span>
                      "{result.mensagem}"
                    </p>
